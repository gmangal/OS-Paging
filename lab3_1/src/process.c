...
void PageFaultHandler()
{
  ...
  int newPage, pageIndex;
	...
	// printf ("It does come to PageFaulthandler\n");
  tempstackframe = currentPCB->currentSavedFrame ;
  faultaddress = tempstackframe[PROCESS_STACK_FAULT];
  pageIndex = faultaddress / MEMORY_PAGE_SIZE;

  if (currentPCB->npages >= 16) {
  ...
  currentPCB->pagetable[pageIndex] = MemorySetupPte (newPage);
  currentPCB->npages++;

  printf("Process id: %d, page fault address: 0x%x, physical page number allocated: %d\n", findpid(currentPCB), faultaddress, pageIndex);
