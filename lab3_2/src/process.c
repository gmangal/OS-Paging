...
//------------------------------------------------------------
  ...
  pcb->npages++;
  (((uint32 *)(pcb->pagetable[0]))[0]) = newPage;

  // clear out first page table
  for (i = 1; i < L2_MAX_ENTRIES; i++) {
      (((uint32 *)(pcb->pagetable[0]))[i]) = 0;
  }
  ...
  pcb->npages++;
  (((uint32 *)(pcb->pagetable[0]))[1]) = newPage;
  ...
  pcb->npages++;
  (((uint32 *)(pcb->pagetable[0]))[2]) = newPage;
  ...
  pcb->npages++;
  (((uint32 *)(pcb->pagetable[L1_MAX_ENTRIES - 1]))[L2_MAX_ENTRIES - 1]) = newPage;

  // clear out last page table
  for (i = 1; i < L2_MAX_ENTRIES; i++) {
      (((uint32 *)(pcb->pagetable[L1_MAX_ENTRIES]))[i]) = 0;
  }
...
//----------------------------------------------
  ...
  if (isUser) {
    ...
    stackframe[PROCESS_STACK_IREG+29] = TOP_VIRTUAL_ADDRESS_SPACE - SIZE_ARG_BUFF;
    ...
    dum[2] = TOP_VIRTUAL_ADDRESS_SPACE - SIZE_ARG_BUFF + offset;
    dbprintf('p', "1\n");
    for(count=3;;count++)
    {
      offset=get_argument(NULL);
      dum[count] = TOP_VIRTUAL_ADDRESS_SPACE - SIZE_ARG_BUFF + offset;
      if(offset==0)
      {
        break;
      }
    ...
    dum[0] = count-2;
    dum[1] = TOP_VIRTUAL_ADDRESS_SPACE - SIZE_ARG_BUFF - (count-2)*4;
...
//  *((int *)0xfff00100) = 't';
  ...
  if (userprog != (char *)0) {
      ...
      process_create(param[0], param[1], param[2], param[3], param[4],
...
void PageFaultHandler()
{
  ...
  int pageIndex, newPage;
  int l1_Index = fault_address >> 19;
  int l2_Index = (fault_address && 0x07FFFF) >> 13;
  ...
  (((uint32 *)(currentPCB->pagetable[l1_Index]))[l2_Index]) = MemorySetupPte(newPage);
  currentPCB->npages++;
  printf("Process id: %d, page fault address: 0x%x, physical page number allocated: %d\n", findpid(currentPCB), fault_address, pageIndex);
